...
- [x] The independent test was forwarded to the Cloudflare-verified Gmail destination; Gmail received it and classified it as Spam. The address is intentionally omitted from tracked documentation.
...
- [x] With forwarding disabled, independent test `CLOUDMAIL-NOFWD-20260715-2` was received as email ID 3 and displayed in Webmail; forwarding was then restored to the verified destination. A Gmail `in:anywhere` search in the authenticated target account returned only `CLOUDMAIL-HTMLATT-20260715-2`, confirming the no-forward subject was absent from Inbox, Spam, Trash, and all other mail.
