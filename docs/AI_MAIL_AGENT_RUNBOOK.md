...
- 运维面板只汇总最近 24 小时运行元数据和 30 天投递成功率；推理失败与投递失败仅保存固定错误类别，不保存错误消息、正文或 Provider 原始响应。
- 每日维护任务记录 AI 专用 D1 表的逻辑字节数和行数；当相较上一快照同时增长超过 10 MB 且超过 2 倍时显示异常增长告警。AI 不读取或写入 R2 附件，因此不会为 AI 伪造 R2 用量指标。
